<?php

header('Content-Type: application/json');
header("Access-Control-Allow-Origin: *");

// Load private key (use ENV in production)
$privateKey = <<<EOD
-----BEGIN PRIVATE KEY-----

NjzGPJuyQ1Nks/mh/4x7
-----END PRIVATE KEY-----
EOD;

//  Request payload
$data = [
    "phone" => $_POST['phone_account']
];

// MUST match Postman exactly
$body = json_encode($data, JSON_UNESCAPED_SLASHES);

//  Sign with SHA256withRSA
$signature = '';
openssl_sign($body, $signature, $privateKey, OPENSSL_ALGO_SHA256);

//  Convert to base64 (same as hextob64)
$authorization = base64_encode($signature);
$authorization = trim($authorization);

//  Optional reference (not used unless required)
$reference = "MBET-OPAY-" . round(microtime(true) * 1000);

//  cURL request
$curl = curl_init();

curl_setopt_array($curl, [
    CURLOPT_URL => 'https://liveapi.opaycheckout.com/api/v1/international/payout/opay-wallet-validate',
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => $body,
    CURLOPT_HTTPHEADER => [
        'MerchantId: 256625012791839',
        'Content-Type: application/json',
        'Authorization: Bearer ' . $authorization
    ],
]);

$response = curl_exec($curl);

//  Error handling
if (curl_errno($curl)) {
    echo json_encode([
        "status" => false,
        "error" => curl_error($curl)
    ]);
    exit;
}

curl_close($curl);

//  Return Opay response
echo $response;